...
const colliders = new QuadTree(new Rectangle({
  x: 768 / 2,
  y: 576 / 2,
  width: 768 * 2,
  height: 576 * 2
...
export class ColliderComponent extends Component<IColliderComponentDesc> implements ILogicComponent {
  ...
  update() {
    ...
    const neighboors = colliders.getNeighboors(this);

    for (const c of neighboors) {
      if (c === this ||
        !c.enabled ||
        !c.owner.active ||
        c.getFlag() == (this.flag & this.mask)) {
          continue;
      }
  ...
  // return the current collider's flag
  getFlag() : number {
    return this.flag;
  }
}
